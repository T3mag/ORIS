...
public class ControllersHandler : Handler {
    public override void HandleRequest(HttpListenerContext context) {
        try {
            ...
            var strParams = request.Url!.Segments.Skip(1).Select(s => s.Replace("/", "")).ToArray();
            var controllerName = strParams[0];
            var methodName = strParams[1];
            var assembly = Assembly.GetExecutingAssembly();
            var controller = assembly.GetTypes().Where(t => Attribute.IsDefined(t, typeof(ControllerAttribute))).FirstOrDefault(c => ((ControllerAttribute)Attribute.GetCustomAttribute(c, typeof(ControllerAttribute))!).ControllerName.Equals(controllerName, StringComparison.OrdinalIgnoreCase));
            var method = controller?.GetMethods().FirstOrDefault(x => x.GetCustomAttributes(true).Any(attr => attr.GetType().Name.Equals($"{request.HttpMethod}Attribute", StringComparison.OrdinalIgnoreCase) && ((HttpMethodAttribute)attr).ActionName.Equals(methodName, StringComparison.OrdinalIgnoreCase)));
            var id =  method!.Name.Equals("Delete") || method.Name.Equals("GetById") || method.Name.Equals("Update")  ? strParams[2] : null;
            var queryParams = method?.GetParameters().Select((p, i) => {
                    if (p.ParameterType == typeof(uint) && i == 0) {
                        return Convert.ChangeType(id, p.ParameterType);
                    }
                    return Convert.ChangeType(strParams[i], p.ParameterType); }).ToArray();
            if (request is { HttpMethod: "POST", HasEntityBody: true } && methodName.Equals("SendToEmail", StringComparison.OrdinalIgnoreCase)) {
