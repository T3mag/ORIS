    ...
    public override void HandleRequest(HttpListenerContext context) {
        try {
            ...
            if (!string.IsNullOrEmpty(data)) {
                var currentOfUserData = data.Split('&');
            ...
            var method = (controller?.GetMethods()!).FirstOrDefault(x => x.GetCustomAttributes(true).Any(attr 
                => attr.GetType().Name.Equals($"{request.HttpMethod}Attribute", StringComparison.OrdinalIgnoreCase) && ((HttpMethodAttribute)attr).Action.Equals(methodName, StringComparison.OrdinalIgnoreCase)));
